...
const Header = ({ carrito }) => {
  ...
  return (
    <>
      {['md'].map((expand) => (
        <Navbar key={expand} expand={expand} className="bg-dark p-3" variant="dark">
          <Container fluid>
            <Navbar.Brand title='Inicio'>
              <HashLink to="/hardware-store"><img src={logo} alt="logo" style={{ height: 50}} /></HashLink>
            </Navbar.Brand>
            ...
            >
              ...
              <Offcanvas.Body>
                ...
                <HashLink to="/hardware-store/carrito" className="m-3 m-md-0 me-md-3 d-flex" style={{ textDecoration: 'none', color: 'inherit' }}>
                  ...
                  {carrito.length > 0 ? 
                    <Badge className="align-self-start border border-light rounded-pill" bg="danger">{carrito.length}</Badge> 
                    : 
                    null}
                </HashLink>
